...
def direct_tunnel_workflow():
    ...
    def generate_direct_rules(new_ports_str=None):
        ...
        rules = [f"# Direct NAT rules v{VERSION}", "", "table inet direct_nat {",
    ...
    def add_direct_tunnel():
        ...
        save_direct_tunnels(tunnels); generate_direct_rules(ports_str)
    ...
    def remove_direct_tunnel():
        ...
        try:
            ...
            if 0 < choice <= len(names):
                ...
                save_direct_tunnels(tunnels); generate_direct_rules()
        except ValueError: print(f"{C.RED}Invalid input.{C.END}")

    while True:
        clear_screen()
        print(f"\n{C.HEADER}--- Direct NAT Tunnel Manager ---{C.END}")
        options = {"1": "Add", "2": "List", "3": "Remove", "4": "Re-apply Rules", "5": "Return"}
        for k, v in options.items(): print(f"{k}. {v} Direct Tunnel")
        choice = input("Enter choice: ").strip()
        if choice == '1': add_direct_tunnel()
        elif choice == '2': list_direct_tunnels()
        elif choice == '3': remove_direct_tunnel()
        elif choice == '4': generate_direct_rules()
        elif choice == '5': break
...
def reverse_tunnel_workflow():
            ...
            if self.path == '/register':
                try:
                    ...
                    if pubkey and re.match(r'^[A-Za-z0-9+/]{42}[AEIMQUYcgkosw048]=$', pubkey):
                        RECEIVED_KEY = pubkey; self.send_response(200); self.end_headers(); self.wfile.write(b'OK')
    ...
    def run_temp_server(holder):
        nonlocal RECEIVED_KEY
        try: holder.append(HTTPServer(('', CALLBACK_PORT), KeyRegHandler)); holder[0].serve_forever()
        except Exception: RECEIVED_KEY = "SERVER_ERROR"

    def get_public_ip():
        for s in ["ifconfig.me/ip", "api.ipify.org"]:
            r = run_command(['curl','-s','--connect-timeout','5',s],False)
            if r and r.stdout and is_valid_ip(r.stdout.strip()): return r.stdout.strip()
        while True:
            ip = input(f"{C.RED}Could not auto-detect IP. Enter manually: {C.END}").strip()
            if is_valid_ip(ip): return ip

    def get_next_wg_ip():
        if not os.path.exists(WG_CONFIG_FILE): return "10.0.0.2"
        with open(WG_CONFIG_FILE, 'r') as f: content = f.read()
        octets = {int(o) for o in re.findall(r'10\.0\.0\.(\d+)', content)}
        o = 2
        while o in octets: o += 1
        return f"10.0.0.{o}" if o <= 254 else None

    def load_peers():
        try:
            with open(REVERSE_PEERS_DB_FILE, 'r') as f: return json.load(f)
        except (IOError, json.JSONDecodeError): return {}

    def save_peers(peers):
        os.makedirs(REVERSE_DB_DIR, exist_ok=True);
        with open(REVERSE_PEERS_DB_FILE, 'w') as f: json.dump(peers, f, indent=4)

    def initial_relay_setup():
        if os.path.exists(WG_CONFIG_FILE) and input("Overwrite existing config? (y/N): ").lower() != 'y': return
        privkey = run_command(['wg','genkey'],False).stdout.strip()
        conf = f"[Interface]\nPrivateKey = {privkey}\nAddress = 10.0.0.1/24\nListenPort = 51820\n"
        os.makedirs(os.path.dirname(WG_CONFIG_FILE), exist_ok=True)
        with open(WG_CONFIG_FILE, 'w') as f: f.write(conf); os.chmod(WG_CONFIG_FILE, 0o600)
        for f in [REVERSE_PEERS_DB_FILE, REVERSE_TUNNELS_DB_FILE]:
            if os.path.exists(f): os.remove(f)
        run_command(['wg-quick','down','wg0']); run_command(['wg-quick','up','wg0'])
        run_command(['systemctl','enable','wg-quick@wg0']); print("Endpoint setup complete.")

    def add_new_peer():
        nonlocal RECEIVED_KEY; RECEIVED_KEY = None
        ip = get_next_wg_ip()
        if not ip: print("Subnet full."); return
        pubkey = run_command(['wg','show','wg0','public-key']).stdout.strip()
        public_ip = get_public_ip()
        holder = []; threading.Thread(target=run_temp_server,args=(holder,),daemon=True).start(); time.sleep(1)
        if RECEIVED_KEY == "SERVER_ERROR": print("Could not start registration server."); return
        ...
        for _ in range(120):
            if RECEIVED_KEY: break
            time.sleep(1)
        if holder: holder[0].shutdown()
        if not RECEIVED_KEY or RECEIVED_KEY == "SERVER_ERROR": print("Timeout or error."); return
        name = input("Enter peer name: ").strip() or f"Peer-{ip}"
        conf = f"\n[Peer]\n# {name}\nPublicKey = {RECEIVED_KEY}\nAllowedIPs = {ip}/32\n"
        with open(WG_CONFIG_FILE, 'a') as f: f.write(conf)
        peers = load_peers(); peers[name] = {'ip': ip, 'pubkey': RECEIVED_KEY}; save_peers(peers)
        run_command(['wg','set','wg0','peer',RECEIVED_KEY,'allowed-ips',f'{ip}/32']); print("Peer added.")

    def remove_peer():
        peers = load_peers()
        if not peers: print("No peers to remove."); return
        names = sorted(peers.keys())
        for i, name in enumerate(names, 1): print(f"{i}. {name}")
        try:
            choice = int(input("Enter number to remove (0 to cancel): "))
            if 0 < choice <= len(names):
                name = names[choice - 1]; pubkey = peers[name]['pubkey']
                with open(WG_CONFIG_FILE, "r") as f: lines = f.readlines()
                with open(WG_CONFIG_FILE, "w") as f:
                    in_peer_section = False
                    for line in lines:
                        if f"PublicKey = {pubkey}" in line: in_peer_section = True; continue
                        if in_peer_section and (line.strip() == "" or line.startswith("[Peer]")): in_peer_section = False
                        if not in_peer_section: f.write(line)
                run_command(['wg','set','wg0','peer',pubkey,'remove'])
                del peers[name]; save_peers(peers); print(f"Peer '{name}' removed.")
    ...
    while True:
        ...
        if not is_setup: print("1. Initial Endpoint Setup");
        else: print("1. Add New Peer"), print("2. Remove Peer")
        print("9. Return to Main Menu")
        choice = input("Enter choice: ").strip()
        if not is_setup and choice == '1': initial_relay_setup(); is_setup = True
        elif is_setup and choice == '1': add_new_peer()
        elif is_setup and choice == '2': remove_peer()
        elif choice == '9': break
...
def setup_wireguard_client(server_pubkey, server_endpoint, callback_url, client_ip):
    ...
    if os.path.exists(WG_CONFIG_FILE): os.remove(WG_CONFIG_FILE)
    privkey = run_command(['wg','genkey'],False).stdout.strip()
    ...
    if not run_command(cmd,False) or run_command(cmd,False).stdout.strip() != 'OK':
        print("Failed to register with endpoint."); sys.exit(1)
    run_command(['wg-quick','up','wg0']); run_command(['systemctl','enable','wg-quick@wg0'])
...
def main():
    ...
    parser.add_argument('--server-pubkey'); parser.add_argument('--server-endpoint')
    parser.add_argument('--callback-url'); parser.add_argument('--client-ip')
    if 'setup_client' in sys.argv:
        ...
        if (args.command == 'setup_client' and all([args.server_pubkey, args.server_endpoint, args.callback_url, args.client_ip])):
            if os.geteuid() != 0: sys.exit("Client setup requires root.")
        ...
        return
    if os.geteuid() != 0: sys.exit("This script requires root privileges.")
    ensure_dependencies({'nftables':'nft', 'curl':'curl', 'wireguard':'wg'})
    ensure_ip_forwarding(); ensure_nftables_service(); ensure_include_line()
    while True:
        clear_screen()
        ...
        print("3. Exit")
        choice = input("Enter choice: ").strip()
        if choice == '1': direct_tunnel_workflow()
        elif choice == '2': reverse_tunnel_workflow()
