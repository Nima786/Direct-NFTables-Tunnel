...
def check_port_conflicts(ports_str, existing_tunnel_ports=None):
    ...
    try:
        ...
        print(f"{C.RED}Error: Invalid port format.{C.END}"); return False
    ...
    if conflicts:
        print(f"{C.RED}Error: Port(s) already in use: {', '.join(map(str, sorted(conflicts)))}{C.END}")
        return False
    return True
...
def ensure_include_line():
    ...
    if not os.path.exists(MAIN_NFT_CONFIG):
        print(f"{C.YELLOW}Main config not found. Creating default...{C.END}")
        default_config = ["#!/usr/sbin/nft -f", "flush ruleset", "", include_line]
        with open(MAIN_NFT_CONFIG, 'w') as f: f.write("\n".join(default_config))
        print(f"{C.GREEN}Default config created.{C.END}")
        return True
    else:
        ...
        if choice == 'y' or choice == '':
            try:
                ...
                print(f"{C.GREEN}Successfully added include line.{C.END}")
                return True
            except Exception as e:
                print(f"{C.RED}Error: Could not write to file. {e}{C.END}"); return False
        else:
            ...
            print(f"\nThen, run: {C.GREEN}sudo systemctl reload nftables{C.END}")
            return False

def apply_nftables_config():
    """Checks if nftables is active, starts it if needed, then reloads."""
    is_active_cmd = ['systemctl', 'is-active', 'nftables']
    is_active_result = subprocess.run(is_active_cmd, capture_output=True, text=True)

    if is_active_result.stdout.strip() != "active":
        print(f"{C.YELLOW}nftables service is not active. Enabling and starting it...{C.END}")
        if not run_command(['systemctl', 'enable', 'nftables']):
            print(f"{C.RED}Failed to enable nftables.{C.END}"); return False
        if not run_command(['systemctl', 'start', 'nftables']):
            print(f"{C.RED}Failed to start nftables.{C.END}"); return False
...
def generate_and_apply_rules(new_ports=None):
    if not ensure_include_line(): return
    ...
    try:
        ...
        public_interface = result.stdout.strip()
    except Exception as e:
        print(f"{C.RED}Error: Could not determine default interface: {e}{C.END}"); return
    ...
    rules_content = [
        f"# NAT rules generated by Tunnel Manager {SCRIPT_VERSION}", "",
    ...
    if apply_nftables_config() and new_ports:
...
def add_new_tunnel():
    ...
    if not check_port_conflicts(ports): return
...
def uninstall_manager():
    """A new function to handle cleanup, separate from the menu."""
    print(f"{C.YELLOW}Uninstalling all components of Tunnel Manager...{C.END}")
    if os.path.exists(TUNNELS_DB_FILE):
        os.remove(TUNNELS_DB_FILE)
        print(f"Removed database file.")
    if os.path.exists(TUNNEL_RULES_FILE):
        os.remove(TUNNEL_RULES_FILE)
        print(f"Removed rules file.")
...
# --- The script is now standalone. No installer. ---
def main_menu():
    if os.geteuid() != 0:
    ...
    while True:
        clear_screen()
        print(f"\n{C.HEADER}===== NFTables Tunnel Manager {SCRIPT_VERSION} =====")
        ...
        print(f"{C.CYAN}6. Uninstall All")
        ...
        elif choice == '6': uninstall_manager(); break
