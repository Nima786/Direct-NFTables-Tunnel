...
def ensure_include_line():
    ...
    if not os.path.exists(MAIN_NFT_CONFIG):
        ...
        if choice == 'y' or choice == '':
            ...
            print(
                "Please add the following line to the end of "
...
def apply_nftables_config():
    ...
    if is_active_result.stdout.strip() != "active":
        ...
        if not run_command(['systemctl', 'start', 'nftables']):
            print(f"{C.RED}Failed to start nftables.{C.END}")
            return False
...
def generate_and_apply_rules(new_ports=None):
    ...
    try:
        ...
        print(f"{C.RED}Error: Could not determine default interface: {e}{C.END}")
        return
