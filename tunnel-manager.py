...
CALLBACK_PORT = 58080
# NOTE: This URL is used by the Endpoint to generate a command for the Client.
# The Client downloads and runs the script from here.
...
def ensure_nftables_service():
    if subprocess.run(['systemctl', 'is-enabled', 'nftables'], capture_output=True, text=True).stdout.strip() != 'enabled':
        run_command(['systemctl', 'enable', 'nftables'])
    if subprocess.run(['systemctl', 'is-active', 'nftables'], capture_output=True, text=True).stdout.strip() not in ['active', 'activating']:
...
def direct_tunnel_workflow():
    ...
    while True:
        ...
        print(f"{C.YELLOW}3. (Edit not implemented in this version)")
        print(f"{C.RED}4. (Remove not implemented in this version)")
...
def reverse_tunnel_workflow():
    ...
    # Global variables for this workflow
    RECEIVED_KEY = None
    
    # --- Helper Functions for Reverse Tunnels ---
    ...
    def get_public_ip():
        ...
        for service in ["ifconfig.me/ip", "api.ipify.org", "icanhazip.com"]:
            ...
            if result and result.stdout and is_valid_ip(result.stdout.strip()):
                ...
                return ip
        while True:
            manual_ip = input(f"{C.RED}Could not auto-detect IP.{C.CYAN} Please enter public IP manually: {C.END}").strip()
    ...
    # --- Main Logic for Reverse Tunnels ---
    
    def initial_relay_setup():
        clear_screen()
        print(f"{C.HEADER}--- Initial WireGuard Endpoint Setup ---{C.END}")
        if os.path.exists(WG_CONFIG_FILE):
            choice = input(f"{C.YELLOW}Config already exists. Overwrite? (This removes all peers): {C.END}").lower().strip()
            if choice != 'y':
                return

        server_b_privkey = run_command(['wg', 'genkey'], use_sudo=False).stdout.strip()
        wg_conf = f"[Interface]\n# Endpoint Server Config\nPrivateKey = {server_b_privkey}\nAddress = 10.0.0.1/24\nListenPort = 51820\n"
        os.makedirs(os.path.dirname(WG_CONFIG_FILE), exist_ok=True)
        with open(WG_CONFIG_FILE, 'w') as f:
            f.write(wg_conf)
        os.chmod(WG_CONFIG_FILE, 0o600)

        if os.path.exists(REVERSE_PEERS_DB_FILE): os.remove(REVERSE_PEERS_DB_FILE)
        if os.path.exists(REVERSE_TUNNELS_DB_FILE): os.remove(REVERSE_TUNNELS_DB_FILE)
    ...
    def add_new_peer():
        ...
        peer_name = input("Enter a descriptive name for this new peer (e.g., germany-1): ").strip() or f"Peer-{peer_ip_to_add}"
    ...
    while True:
        ...
        print(f"{C.GREEN}2. (Add Forwarding Rule - Not Implemented)")
        print(f"{C.CYAN}3. (List Rules - Not Implemented)")
        print(f"{C.YELLOW}4. (Edit Rule - Not Implemented)")
        print(f"{C.RED}5. (Remove Rule - Not Implemented)")
        
        print(f"\n{C.YELLOW}--- System ---{C.END}")
        print(f"{C.YELLOW}6. (Re-apply All Rules - Not Implemented)")
        print(f"{C.YELLOW}7. (Uninstall - Not Implemented)")
        ...
        if choice == '1':
            if not is_setup:
                initial_relay_setup()
                is_setup = True # Update state after setup
            else:
                add_new_peer()
        elif choice == '8':
            break
...
def main():
    ...
    # Argument parsing for client setup command
    ...
    # Check if 'setup_client' command is being run
    if 'setup_client' in sys.argv:
        ...
        # sys.exit(0) is called within the function, so we don't need it here.
        return # Exit after client setup
