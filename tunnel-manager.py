...
def main_menu():
    ...
    if not os.path.exists(INSTALL_PATH):
        clear_screen()
        print(f"{C.HEADER}===== Welcome to Tunnel Manager Setup {SCRIPT_VERSION} =====")
...
if __name__ == '__main__':
    if sys.stdin.isatty():
        ...
        print("Running initial setup from pipe...")
        script_content = sys.stdin.read()
        temp_script_path = "/tmp/tunnel_manager_installer.py"
        with open(temp_script_path, 'w') as f:
            f.write(script_content)
        
        os.chmod(temp_script_path, 0o755)
        subprocess.call(['sudo', 'python3', temp_script_path])
